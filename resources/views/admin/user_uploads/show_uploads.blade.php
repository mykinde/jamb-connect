<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Admin: Uploads for') }} {{ $user->name }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-medium text-gray-900">
                            Exam Uploads for <span class="font-bold text-indigo-700">{{ $user->name }} ({{ $user->email }})</span>
                        </h3>{{ $user->number }}
                        <a href="{{ route('admin.user_uploads.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-500 active:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            {{ __('Back to Users List') }}
                        </a>
                    </div>

                    <form action="{{ route('admin.user_uploads.show', $user->id) }}" method="GET" class="mb-6 p-4 border rounded-md bg-gray-50">
                    ...
                    @if ($uploads->isEmpty())
                        <p class="text-gray-600">No uploads found for this user matching your criteria.</p>
                    @else
                                    ...
                                    @foreach ($uploads as $upload)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                {{-- Link to view the specific upload's details. Admin can view anyone's upload details --}}
                                                <a href="{{ route('uploads.show', $upload->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                                {{-- Admin can edit/delete any upload directly --}}
