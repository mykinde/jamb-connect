        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Edit Your Account Profile</h3>
                    <p class="text-sm text-gray-600 mb-6">Update your account details below.</p>
                    ...
                    <form method="POST" action="{{ route('accounts.update', $account->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="profile_code" class="block mt-1 w-full" type="text" name="profile_code" :value="old('profile_code', $account->profile_code)" required autofocus />
                        ...
                        <div class="mb-4">
                            ...
                            <select id="application_type" name="application_type" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach($applicationTypes as $type)
                                    <option value="{{ $type }}" {{ old('application_type', $account->application_type) == $type ? 'selected' : '' }}>
                                        {{ $type }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone', $account->phone)" />
                            <x-input-error :messages="$errors->get('phone')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="email_address" :value="__('Contact Email Address')" />
                            <x-text-input id="email_address" class="block mt-1 w-full" type="email" name="email_address" :value="old('email_address', $account->email_address)" required />
                        ...
                        <div class="mb-4">
                            ...
                            <select id="last_jamb_year" name="last_jamb_year" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($jambYears as $year)
                                    <option value="{{ $year }}" {{ old('last_jamb_year', $account->last_jamb_year) == $year ? 'selected' : '' }}>
                                        {{ $year }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="last_institution_attended" class="block mt-1 w-full" type="text" name="last_institution_attended" :value="old('last_institution_attended', $account->last_institution_attended)" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="proposed_new_institution" class="block mt-1 w-full" type="text" name="proposed_new_institution" :value="old('proposed_new_institution', $account->proposed_new_institution)" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="proposed_course" class="block mt-1 w-full" type="text" name="proposed_course" :value="old('proposed_course', $account->proposed_course)" />
                        ...
                        <div class="mb-4">
                            ...
                            <select id="application_year" name="application_year" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($applicationYears as $year)
                                    <option value="{{ $year }}" {{ old('application_year', $account->application_year) == $year ? 'selected' : '' }}>
                                        {{ $year }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="nationality" class="block mt-1 w-full" type="text" name="nationality" :value="old('nationality', $account->nationality)" />
                            <x-input-error :messages="$errors->get('nationality')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="state" :value="__('State (Optional)')" />
                            <x-text-input id="state" class="block mt-1 w-full" type="text" name="state" :value="old('state', $account->state)" />
                            <x-input-error :messages="$errors->get('state')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="lga" :value="__('LGA (Local Government Area) (Optional)')" />
                            <x-text-input id="lga" class="block mt-1 w-full" type="text" name="lga" :value="old('lga', $account->lga)" />
                            <x-input-error :messages="$errors->get('lga')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="resident_address" :value="__('Resident Address (Optional)')" />
                            <x-text-input id="resident_address" class="block mt-1 w-full" type="text" name="resident_address" :value="old('resident_address', $account->resident_address)" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="resident_town" class="block mt-1 w-full" type="text" name="resident_town" :value="old('resident_town', $account->resident_town)" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="resident_state" class="block mt-1 w-full" type="text" name="resident_state" :value="old('resident_state', $account->resident_state)" />
                        ...
                        <div class="mb-4">
                            ...
                            <select id="marital_status" name="marital_status" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($maritalStatuses as $status)
                                    <option value="{{ $status }}" {{ old('marital_status', $account->marital_status) == $status ? 'selected' : '' }}>
                                        {{ $status }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            <label for="blind" class="inline-flex items-center mr-4">
                                <input id="blind" type="checkbox" name="blind" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('blind', $account->blind) ? 'checked' : '' }}>
                                <span class="ml-2 text-sm text-gray-600">{{ __('Blind') }}</span>
                            </label>
                            <label for="deaf" class="inline-flex items-center mr-4">
                                <input id="deaf" type="checkbox" name="deaf" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('deaf', $account->deaf) ? 'checked' : '' }}>
                                <span class="ml-2 text-sm text-gray-600">{{ __('Deaf') }}</span>
                            </label>
                            <label for="dumb" class="inline-flex items-center mr-4">
                                <input id="dumb" type="checkbox" name="dumb" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('dumb', $account->dumb) ? 'checked' : '' }}>
                                <span class="ml-2 text-sm text-gray-600">{{ __('Dumb') }}</span>
                            </label>
                            <label for="lame" class="inline-flex items-center">
                                <input id="lame" type="checkbox" name="lame" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('lame', $account->lame) ? 'checked' : '' }}>
